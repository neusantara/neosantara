...
let handler = async (m, {
	...
	let user = db.data.users[m.sender]
	  if (new Date - user.lastberbru < cooldown) {
	   conn.reply(m.chat, `*Sepertinya Kamu Sudah Kecapekan*\n*Silahkan Istirahat Dulu Selama* ${((user.lastberbru + cooldown) - new Date()).toTimeString()}*`, m)
	   } else {
...
`
		...
		setTimeout(() => {
			conn.reply(m.chat, `Duarrrr, Kamu Adu Mekanik Dengan Buruanmu Di Exp Lane`, m)
		}, 15000)
